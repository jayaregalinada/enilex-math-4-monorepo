...
import { IconNextTrack } from './icon-next-track';

describe('IconNextTrack', () => {
  it('renders a decorative svg with a shape child by default', () => {
    const { container } = render(<IconNextTrack />);
  ...
  it('exposes an img role labelled with the title prop', () => {
    render(<IconNextTrack title="Next track" />);

    // a11y: a titled icon becomes an image labelled for assistive tech.
    const icon = screen.getByRole('img');
    expect(icon).toHaveAttribute('aria-label', 'Next track');
  });
});
